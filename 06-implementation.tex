...
public async merge(log) {
    ...
    \end{lstlisting}
    \caption{\todo{Zapisat algoritmom. Nie kodom}Algorithm for merging database logs}
...
After ExplorerAPI starts, it connect to the all enabled cryptocurrency databases (specified in \texttt{.env} file). Then it register all three controllers (for blocks, transactions and addresses) and starts HTTP server on port described in \texttt{.env} file or \texttt{5000} if there is no specific port in the environment file. ExplorerAPI has only three endpoints (\texttt{block}, \texttt{address} and \texttt{transaction}). All these endpoints accepts query param (in case of \texttt{GET} HTTP method) or body param (\texttt{POST} HTTP method) in which query is described (see Figure \ref{explorerApiRequest}). This param can consists of array of conditions and filters, skip property and resolver. The param is translated to the Query system and result is return in JSON format (see Figure \ref{explorerApiResponse}). ExplorerAPI can accept multiple connections at once, because each request is processed in separated promise (task).
